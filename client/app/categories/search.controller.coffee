...
.controller 'SearchCtrl', ($scope, $state, $stateParams, $http, $q) ->
  ...
  $scope.nothingFound = ->
    _.all _.keys($scope.results).forEach (key) ->
      $scope.results[key].empty
  ...
  $scope.results = {}
  $scope.selected.selection.forEach (id) ->
    $scope.results[id] = {
      loaded: false
      ads: []
    }
    $scope.getResults id, $stateParams
    .then (results) ->
      $scope.results[id].loaded = true
      $scope.results[id].empty = results.length == 0
